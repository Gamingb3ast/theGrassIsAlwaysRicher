            ...
            if(args.length > 0) {
                if (args[0].equals("simulateGrassPatches")) {
                    try {
                        ...
                        ChatComponentText message = new ChatComponentText("Beginning Grass patch simulation... Amount of patches:: " + amount);
                        ...
                        ChatComponentText message = new ChatComponentText("Second argument needs to be an integer! Check command usage");
                        message.getChatStyle().setColor(EnumChatFormatting.RED);
                        player.addChatMessage(message);
                    }
    ...
    public List<String> addTabCompletionOptions(ICommandSender sender, String[] args) {
        switch (args.length) {
            ...
            case 2:
                ...
                if(args[0].equals("setConfigValue"))
                    return Config.configuration.getCategory("general").getPropertyOrder();
                if(args[0].equals("simulateGrassPatches"))
                    return Arrays.asList("0", "1", "2", "3", "4","5", "etc.");
                if(args[0].equals("getConfigValue"))
                    return Config.configuration.getCategory("general").getPropertyOrder();
        }
        return null;
    }
