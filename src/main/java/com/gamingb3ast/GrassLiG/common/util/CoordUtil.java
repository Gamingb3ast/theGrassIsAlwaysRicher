...
public class CoordUtil {
    ...
    public int getTopBlockTypeY(World w, Block block) {
        int k;
        // todo: instead of configurable k value, have it be the player y level and then check a few blocks in either
        // direction (configurable distance)
    ...
    public CoordUtil getRandomCoordInRange(int diameter) {
        return this.addXZ(
            (int) (Math.random() * diameter) - (diameter / 2),
            (int) (Math.random() * diameter) - (diameter / 2));
    }
