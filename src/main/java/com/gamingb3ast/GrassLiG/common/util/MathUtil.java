        ...
        switch (spreadMethod) {
            case "RANDOM":
                for (int i = 0; i < num; i++) {
                    ...
                    queue.add(unitCircleCoords(radius, radian).addXZ((int) player.posX, (int) player.posZ));
                }
                break;
            case "SPLIT":
            ...
            case "PATCHES":
                ...
                int size = (Math.sqrt(num) - (int) Math.sqrt(num) >= 0.5 ? (int) (Math.sqrt(num) + 1)
                    : (int) Math.sqrt(num));
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    for (int j = 0; j < size; j++) {
                        queue.add(
                            unitCircleCoords(radius, radian).addXZ((int) player.posX, (int) player.posZ)
                                .getRandomCoordInRange(patchDiameter));
                    }
                }

            case "SEQUENTIAL":
                // Grabs num amount of initial coordinates, then spreads from those coordinates
                // Each next patch will be a maximum of a certain distance away from previous (Use same
                // com.gamingb3ast.GrassIsRicher.common.config variable as size of patch)
